...
public void activateServerSystem() {
  ...
  if (uberState == uberStates.USILENCE) {
    ...
    float mappedAverage = map(currentAverage, 0.003, 0.09, 5, 510);
    ...
    float mappedAverage = map(currentAverage, 0.001, 0.1, 5, 510);
      ...
      println("WIGGLING");
      int potValue1 = getPotValue(potInstallation2);
        println("podvalue ding is " + potValue1);
        arduino.digitalWrite(installation2, Arduino.LOW);

      if (potValue1 > 400) { 
        arduino.digitalWrite(installation2, Arduino.LOW);
      } else if (potValue1 < 300) {
        arduino.digitalWrite(installation2, 3);
      }
    }    
  }
